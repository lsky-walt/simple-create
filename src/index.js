...
const install = async (projectName, useNPM) => {
  ...
  if (useNPM) {
    ...
    await promiseSpawn({
      command: "npm",
      args: ["install", "--save", "--save-exact", "--prefix", root],
    })
    return
  }
...
const createFromOnline = async (...args) => {
  const [projectName, basePath, online, useNPM, template] = args
  ...
  // 强制要求 packages/template
  await fse.copy(path.join(t, `packages/${template}`), basePath)
...
const createFromBase = async (...args) => {
  const [projectName, basePath, useNPM, template] = args
...
const generate = async (...args) => {
  const [projectName, useNPM, online, template] = args
  ...
  if (online && typeof online === "string") {
    ...
    // online is url
    console.log(
      `Use ${stressWarn("\u007B online \u007D")} asset: ${info(online)}.`
    )
    console.log()

    await createFromOnline(projectName, basePath, online, useNPM, template)
